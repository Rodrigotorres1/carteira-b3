...
def calcular_score_radar(dados: dict, selic: float) -> dict:
    ...
    if pl is not None and pl < 0:
        score -= 3
        penalidades.append(f"P/L negativo ({pl:.1f}): empresa com prejuízo. Alto risco.")
    if pvp is not None and pvp > 3.0:
        score -= 1
        penalidades.append(f"P/VP muito elevado ({pvp:.2f}): prêmio alto sobre o patrimônio.")
    if pvp is not None and pvp < 0.5 and pl is not None and pl > 20:
        score -= 2
        penalidades.append(
            f"P/VP muito baixo ({pvp:.2f}) com P/L alto ({pl:.1f}): pode indicar destruição de patrimônio."
        )

    if estilo == "Valor":
        if dy is not None and dy >= selic:
            score += 3
            justificativas.append(f"DY de {dy:.1f}% supera a Selic ({selic:.1f}%). Boa geração de renda.")
        elif dy is not None and dy >= selic * 0.8:
            score += 1
            justificativas.append(f"DY de {dy:.1f}% próximo da Selic ({selic:.1f}%).")
        elif dy is not None:
            penalidades.append(f"DY de {dy:.1f}% abaixo da Selic. Pouco atrativo para renda.")
        ...
        if pvp is not None and pvp < 1.0:
            score += 2
            justificativas.append(f"P/VP de {pvp:.2f}: cota abaixo do valor patrimonial.")
        ...
        if roe is not None and roe > 25:
            score += 3
            justificativas.append(f"ROE de {roe:.1f}%: retorno sobre patrimônio excelente.")
        ...
        if upside is not None and upside > 20:
            score += 3
            justificativas.append(f"Upside de +{upside:.1f}%: desconto significativo em relação ao alvo.")
            ...
            penalidades.append("Sem upside segundo analistas.")
        if pl is not None and pl > 0:
            justificativas.append(f"P/L de {pl:.1f}: típico de crescimento. Avalie pelo ROE.")
